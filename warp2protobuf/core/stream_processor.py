...
class StreamProcessor:
    ...
    async def create_stream_session(self, stream_id: str, message_type: str = "warp.multi_agent.v1.Response") -> 'StreamSession':
        ...
        logger.debug(f"创建流式会话: {stream_id}, 消息类型: {message_type}")
        return session
    ...
    async def close_stream_session(self, stream_id: str):
        ...
        if stream_id in self.active_streams:
            ...
            logger.debug(f"关闭流式会话: {stream_id}")
...
class StreamSession:
    ...
    async def finalize(self) -> Dict[str, Any]:
        ...
        logger.debug(f"流式会话 {self.session_id} 完成: {self.chunk_count} 块, 总大小 {self.total_size} 字节, 耗时 {duration:.2f}s")
        ...
        try:
            ...
            logger.debug(f"流式消息拼接成功: {len(complete_data)} 字节")
